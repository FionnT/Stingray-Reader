...
try:  # pragma: no cover
    ...
    from jsonschema import Draft7Validator as SchemaValidator  # noqa: F401
    ...
    def __init__(self, name: Union[Path, str], **kwargs: Any) -> None:
        ...
        match name:
            ...
            case _:  # pragma: no cover
                raise ValueError(f"must be str or Path: {name!r}")
        self.kwargs = kwargs
...
class COBOL_EBCDIC_Sheet(Sheet[NDInstance]):
    ...
    def row_iter(self) -> Iterator["Row[NDInstance]"]:
        ...
        """
        wb = cast(COBOL_EBCDIC_File, self.workbook())
        self.raw_instance_iter = cast(EBCDIC, wb.unpacker).instance_iter(
            self.name, recfm_class=wb.recfm_class, lrecl=self.lrecl
