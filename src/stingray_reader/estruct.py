...
class Representation(NamedTuple):
    ...
    def pattern(self) -> str:
        ...
        for elt in self.picture_elements:
            if sign := elt.get("sign", ""):
                ...
                # The PICTURE clause was not normalized.
                raise DesignError(
                    f"failure to normalize picture {self.picture_elements!r}"
                )
            else:  # pragma: no cover
                # The regex pattern and the if statement don't match.
                raise DesignError(
                    f"unexpected {elt} in picture {self.picture_elements!r}"
                )
        return "".join(regexp)
    ...
    def zoned_decimal(self) -> bool:
        ...
        edit_options: list[str] = list(
            filter(None, (x.get("char") for x in self.picture_elements))
        )
        numeric_options = (
            self.picture_size
            != 0,  # If PIC omitted, size will be zero, and it's a GROUP element.
...
def unpack(format: str, buffer: bytes) -> tuple[Any, ...]:
    ...
    if representation.usage in ("DISPLAY",):
        ...
        if not re.match(representation.pattern, text):
            raise ValueError(
                f"{text!r} doesn't match pattern {representation.pattern!r}"
            )
        # It's display text.
        return (text,)
...
def calcsize(format: str) -> int:
    ...
    if representation.usage in ("DISPLAY",):
        ...
        return (
            2
            if representation.picture_size < 5
            else (4 if 5 <= representation.picture_size < 10 else 8)
        )
    else:
...
class RECFM_N(RECFM_Reader):
    ...
    def record_iter(self) -> Iterator[bytes]:
        ...
        while len(self.buffer) != 0:
            ...
            if self._used == 0:
                raise RuntimeError(
                    "no bytes consumed from buffer via the .used() method"
                )
